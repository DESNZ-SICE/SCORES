...
hydrogensensitivity = "low"
gassenstivity = "medium"

runname = f"{gassenstivity}gas_{hydrogensensitivity}"
...
x = System_LinProg_Model(
    ...
    # dispatchable_list=[Unabatedgasgen, Gasgen],
    # dispatchable_energy_limits=[0.05, 1],
    dispatchable_list=[Gasgen],
    # dispatchable_energy_limits=[1],
)
...
b = (
    sum(
        (pyo.value(x.model.timehorizon) / (365 * 24))
        * pyo.value(x.model.GenCapacity[g])
        * pyo.value(x.model.GenCosts[g, 0])
        + pyo.value(x.model.GenCapacity[g])
        * pyo.value(x.model.GenCosts[g, 1])
        * sum(pyo.value(x.model.NormalisedGen[g, t]) for t in x.model.TimeIndex)
        for g in x.model.GenIndex
    )
    + sum(
        (pyo.value(x.model.timehorizon) / (365 * 24))
        * np.max([pyo.value(x.model.DispatchedPower[d, t]) for t in x.model.TimeIndex])
        * pyo.value(x.model.DispatchableCosts[d, 0])
        + pyo.value(x.model.DispatchableCosts[d, 1])
        * sum(pyo.value(x.model.DispatchedPower[d, t]) for t in x.model.TimeIndex)
        for d in x.model.DispatchableIndex
    )
    + sum(
        (pyo.value(x.model.timehorizon) / (365 * 24))
        * (
            pyo.value(x.model.StorCosts[i, 0]) * pyo.value(x.model.BuiltCapacity[i])
            + np.max([pyo.value(x.model.C[i, t]) for t in x.model.TimeIndex])
            / (x.Mult_Stor.assets[i].eff_in / 100.0)
            * pyo.value(x.model.StorChargeCosts[i, 0])
            + np.max([pyo.value(x.model.D[i, t]) for t in x.model.TimeIndex])
            * (x.Mult_Stor.assets[i].eff_out / 100.0)
            * pyo.value(x.model.StorDischargeCosts[i, 0])
        )
        for i in x.model.StorageIndex
    )
    + sum(
        sum(
            (pyo.value(x.model.StorCosts[i, 1]) + 0.05) * pyo.value(x.model.D[i, t])
            + (
                pyo.value(x.model.StorChargeCosts[i, 1])
                * pyo.value(x.model.C[i, t])
                / (x.Mult_Stor.assets[i].eff_in / 100.0)
            )
            + (
                pyo.value(x.model.StorDischargeCosts[i, 1])
                * pyo.value(x.model.D[i, t])
                / (x.Mult_Stor.assets[i].eff_out / 100.0)
            )
            for t in x.model.TimeIndex
        )
        for i in x.model.StorageIndex
    )
    + sum(
        sum(
            (pyo.value(x.model.timehorizon) / (365 * 24))
            * pyo.value(x.model.chargercost[k, b])
            * pyo.value(x.model.EV_TypeBuiltCapacity[k, b])
            for b in x.model.ChargeType
        )
        for k in x.model.FleetIndex
    )
    + sum(
        pyo.value(x.model.fossilfuelpenalty) * pyo.value(x.model.Pfos[t])
        for t in x.model.TimeIndex
    )
    + sum(
        sum((0.05) * pyo.value(x.model.EV_D[k, t, 0]) for t in x.model.TimeIndex)
        for k in x.model.FleetIndex
    )
)

# %%
numberofyears = x.model.timehorizon / (365 * 24)
# %%
print(b / (numberofyears * 10**6))
# %%
