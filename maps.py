...
from generation import (
    ...
    OffshoreWindModel12000,
    OffshoreWindModel15000,
    OffshoreWindModel17000,
    OffshoreWindModel20000,
    OnshoreWindModel,
    OnshoreWindModel3600,
    OnshoreWindModel2000,
    OnshoreWindModel3000,
    OnshoreWindModel4000,
    OnshoreWindModel5000,
    OnshoreWindModel6000,
    OnshoreWindModel7000,
    SolarModel,
    ...
    def store_results(self):
        with open(self.filepath, "w") as csvfile:
            ...
            for n in self.load_factors:
                writer.writerow(
                    self.load_factors[n][:2] + [self.load_factors[n][2] / 100]
                )
    ...
    def calculate_correlation_factors(self):
        ...
        for site in locs:
            ...
            try:
                p = (
                    gm.power_out
                )  # might make sense to include this as a function in Generation.py rather than here
                correl = np.corrcoef(self.comparator, p)  # Pearson coefficient
                print(
                    "Site " + str(site) + " has a correlation of " + str(correl[0][1])
                )
            except:
                continue
...
class LoadFactorMap:
    ...
    def draw_map(
        self, show=True, savepath="", cmap=None, vmax=None, vmin=None, turb=None
    ):  # CQ edit: turb=None
        if cmap is None:
            ...
            for j in range(len(y)):
                ...
                if LC.is_land_check(xpt, ypt) == self.is_land:
                    if self.is_land is True:
                        # Ireland
                        if (xpt < 200000) and (ypt < 930000) and (ypt > 340000):
                            continue
                        # France
        ...
        if (
            turb is not None
        ):  # CQ edit to include this if statement, scaling load factor to power generation
...
class OnshoreWindMap(LoadFactorMap):
    def __init__(
        self, lat_num=400, lon_num=300, quality="h", turbine_size=3.6, data_loc=None
    ):
        lfe = LoadFactorEstimator("w" + str(float(turbine_size)), data_loc=data_loc)
