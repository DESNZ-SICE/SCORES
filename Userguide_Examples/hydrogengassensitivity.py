...
for hydrogensensitivity in hydrogenranges:
    for gassenstivity in gasranges:
        ...
        # Store Results
        x.df_capacity.to_csv(
            f"log/internalsensitivites/{runname}Capacities.csv", index=False
        )
        x.df_costs.to_csv(f"log/internalsensitivites/{runname}Costs.csv", index=False)
# %%
