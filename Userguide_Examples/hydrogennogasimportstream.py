...
xlinksgenerator = GenerationModel(
    [1], yearmin, yearmax, list(range(1, 13)), 0, 0, 101, "Xlinks", "", "", [0, 3600]
)


xlinksdata = np.loadtxt(
    "/Users/matt/SCORESdata/demand_xlinks3-6.csv", delimiter=",", skiprows=1, usecols=2
)
...
xlinkdata = xlinksdata[startindex:endindex]
for i in range(len(xlinkdata)):
    xlinksgenerator.power_out[i] = xlinkdata[i]
...
print(f"Offshore LCOE is {offshoregenerator.calculate_LCOE()}")
print(f"Offshore load factor is {offshoregenerator.get_load_factor()}")
...
print(f"Onshore LCOE is {onshoregenerator.calculate_LCOE()}")
print(f"Onshore load factor is {onshoregenerator.get_load_factor()}")
...
print(f"Solar LCOE is {solar_generator.calculate_LCOE()}")
...
x.Run_Sizing(solver="gurobi", timelimit=2000)

# %%
# %%
runname = "xlinksvhighhydrogen"
# Store Results
x.df_capacity.to_csv(f"log/gashydrogenrange99.999/{runname}Capacities.csv", index=False)
x.df_costs.to_csv(f"log/gashydrogenrange99.999/{runname}Costs.csv", index=False)
# %%
import pyomo.environ as pyo

HydrogenSOC = [pyo.value(x.model.SOC[1, t]) / 1000 for t in x.model.TimeIndex]
# hydrogenchargetimeseries = [
#     pyo.value(x.model.C[1, t])
#     / ((hydrogencosts[hydrogensensitivity]["eff_in"]) / 100)
#     / 1000
#     for t in x.model.TimeIndex
# ]
# %%
plt.plot(HydrogenSOC)
plt.ylabel("Hydrogen Storage (TWh)")
# put years on the x axis instead of hours
plt.xticks(
    ticks=np.arange(0, numberofpoints, 24 * 365),
    labels=np.arange(yearmin, yearmax + 2),
)
# plt.plot(demand/1000)
# %%
